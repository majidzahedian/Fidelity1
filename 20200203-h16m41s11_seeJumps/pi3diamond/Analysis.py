...
class Trace(BaseTrace):
    ...
    def guess_threshold(self, trace):
        ...
        hist_binary = np.array(hist) > max(hist) * 0.1
    ...
    def calc_threshold(self, trace, plot=False):
        ...
        for i in range(len(function) - 1):
            if function[i] < 0 and function[i + 1] >= 0:
                ...
                root_index = i + 1
                break
        threshold = fp['bin_edges'][root_index]
    ...
    def append_digital(self, df):
        ...
        for idx, step in enumerate(self.analyze_sequence):
            ...
            df.loc[df['step'] == idx, 'reliable'] = np.abs(diff).reshape(-1) > df.loc[df['step'] == idx, 'thr_diff']
    ...
    def analyze(self):
        df = self.df_complete()
        data_result = dh.Data(parameter_names=['sm', 'step', 'result_num'], observation_names=['result', 'events', 'thresholds'], dtypes=collections.OrderedDict([('result', 'float'), ('events', 'int'), ('thresholds', 'int')]))
        data_result.init()
            ...
            for _, df_step_i in df_step.iterrows():
                ...
                if self.analyze_type == 'consecutive':
                    ...
                    for result_num, row in r.iterrows():
                        data_result.df.loc[(data_result.df.sm==sm) & (data_result.df.step==df_step_i['step']) & (data_result.df.result_num==result_num), ['result', 'events']] = r.loc[result_num, ['mean', 'count']].values
        ...
        if self.average_results:
            data_result.eliminate_parameter(parameter_name='result_num', observation_names=['result', 'events', 'thresholds'], operations=[dh.weight_mean(data_result.df['events']), np.sum, np.mean])
        return data_result
