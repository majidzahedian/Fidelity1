...
#     comment = misc.ret_property_typecheck('comment', str)
#
#     @property
#     def parent(self):
...
#
#     phase_offset_type = misc.ret_property_list_element('phase_offset_type', ['coherent', 'absolute'])
#     smpl_marker = misc.ret_property_typecheck('smpl_marker', bool)
#     sync_marker = misc.ret_property_typecheck('sync_marker', bool)
#
#     @property
#     def length_mus(self):
...
#         if val is not None:
#             valid_length_mus(val)
#             self._length_mus = misc.check_range(misc.check_type(val, 'length_mus', Number), 'length_mus', 0, (__MAX_LENGTH_SMPL__-1) / __SAMPLE_FREQUENCY__)
#             self._length_smpl = length_mus2length_smpl(self._length_mus)
#
#     @length_smpl.setter
#     def length_smpl(self, val):
#         if val is not None:
#             self._length_mus = misc.check_range(misc.check_type(val, 'length_smpl', Number), 'length_smpl', 0, __MAX_LENGTH_SMPL__-1) / __SAMPLE_FREQUENCY__
#             self._length_smpl = length_mus2length_smpl(self._length_mus)
#
#     @property
#     def wave_file(self):
...
#     @type.setter
#     def type(self, val):
#         self._type = misc.check_list_element(val, 'type', ['wait', 'sine', 'robust'])
...
#         self._frequencies = np.array(misc.check_array_like_typ(val, 'frequencies', Number), dtype=float)
...
#     @amplitudes.setter
#     def amplitudes(self, val):
#         self._amplitudes = np.array(misc.check_array_like_typ(val, 'amplitudes', Number), dtype=float)
...
#     @phases.setter
#     def phases(self, val):
#         self._phases = np.array(misc.check_array_like_typ(val, 'phases', Number), dtype=float)
...
#
#     loop_count = misc.ret_property_range('loop_count', int, 0, 2 ** 32 - 1)
#     advance_mode = misc.ret_property_list_element('advance_mode', ['AUTO', 'COND', 'REP', 'SING'])
#
#
...
#         for i in misc.check_type(val, 'data_list', (list, DataList)):
...
#         self._marker_enable = misc.check_type(val, 'marker_enable', bool)
#         self.write_sequence_memory = True
#
...
#         if val is not None:
#             valid_length_mus(val)
#             self._segment_start_offset_mus = misc.check_range(misc.check_type(val, 'segment_start_offset_mus', Number), 'segment_start_offset_mus', 0, __MAX_LENGTH_SMPL__ / __SAMPLE_FREQUENCY__)
...
#         if val is not None:
#             valid_length_mus(val)
#             self._segment_end_offset_mus = misc.check_range(misc.check_type(val, 'segment_end_offset_mus', Number), 'segment_end_offset_mus', 0, __MAX_LENGTH_SMPL__ / __SAMPLE_FREQUENCY__)
...
#             pi3d.logger.debug("type {} {} precompiled ({} s).".format(self.name, type(self), time.time() - t0))
#
...
#
#     date = misc.ret_property_typecheck('date', str)
#
#     @property
#     def missing_smpl(self):
...
#     fs = 12.e3
#     x = np.arange(fs*10)/fs
#     def fr(x, amp=None):
#         return np.around(f(x=x, amp=amp) * 2. ** 12.) / 2. ** 12.
#
#     def f(x, amp=None):
#         return amp * .5 * np.sin(2 * np.pi * x)
#     # xfft, yfft = (np.fft.rfftfreq(len(x) + zpad, x[1] - x[0]), np.fft.rfft(np.hstack([y - y.mean(), np.zeros([zpad])])))
#     # yfftf = np.array(np.abs(yfft) ** 2, dtype=np.float)
#     # plt.plot(xfft[:100], yfftf[:100])
#
#
...
#
#     amp = 0.0005
#     xp, p = ss.periodogram(f(x, amp), fs)
#     xpr, pr = ss.periodogram(fr(x, amp), fs)
