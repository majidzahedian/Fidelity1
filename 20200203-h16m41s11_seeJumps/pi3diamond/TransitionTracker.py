...
class TransitionTracker(QMainWindow, transition_tracker_gui.Ui_window):
    ...
    def set_h_diag(self):
        self.nvham = NVHam(magnet_field={'z': self.current_magnetic_field}, n_type='14n', nitrogen_levels=[0, 1, 2], electron_levels=[0, 1, 2], gamma=self.g_factors, qp=self.qp, hf_para_n=self.hf_para_n, hf_perp_n=self.hf_perp_n)
    ...
    def mw_mixing_frequency(self, val):
        ...
        if getattr(self, '__mw_mixing_frequency', 0.0) != 0.0:
            pi3d.save_value_to_file(self.mw_mixing_frequency, 'mw_mixing_frequency')
            pi3d.save_values_hdf(classifier='mw_mixing_frequency', vd=dict(none=self.mw_mixing_frequency))
        self.update_stuff()
    ...
    def update_stuff(self):
        for attr_name in ['_current_local_oscillator_freq', '_mw_mixing_frequency', '_zero_field_splitting']:
    ...
    def get_rabi_parameter(self, name, **kwargs):
        ...
        if 'e_rabi' in name and not '_ou' in name:
            name = "{}_ou{:.0f}deg{}".format(name, 1000*pi3d.mcas_dict.awgs['2g'].ch[1].output_amplitude, kwargs['mixer_deg'])
    ...
    def rp(self, name, **kwargs):
        if len(kwargs) == 0:
            ...
            return self.rabi_parameters['e_rabi_ou{:.0f}deg{}'.format(1000*pi3d.mcas_dict.awgs['2g'].ch[1].output_amplitude, kwargs['mixer_deg'])]
        else:
            return self.get_rabi_parameter(name, **kwargs)

    @property
    def current_magnetic_field(self):
